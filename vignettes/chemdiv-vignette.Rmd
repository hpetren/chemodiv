...
The user is intentionally required to gather the IDs manually to ensure 
...
NPClassifier tries to classify phytochemical compounds into a hierarchical
classification of three groups (pathway, superclass and class), largely 
corresponding to their biosynthesis.
```{r}
...
*proportions* dataset. The function can calculate a 
...
as columns and each sample as rows.
...
network above. The boxplot indicates that the functional Hill diversity
of the floral scent compounds is highest in the G1 population (further
analyses can specify more exactly what components of diversity is higher in 
